      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", gap: "16px", flexWrap: "wrap" as const }}>
            <span style={{ display: "flex", alignItems: "center", gap: "5px", fontSize: "12px", color: "#9a9690", fontFamily: "var(--font-sans)" }}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
              Dibuka: {formatDate(item.published_at)}
            </span>
            {item.expires_at && (
              <span style={{ display: "flex", alignItems: "center", gap: "5px", fontSize: "12px", color: expiring ? "#d97706" : "#9a9690", fontWeight: expiring ? "600" : "400", fontFamily: "var(--font-sans)" }}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                Ditutup: {formatDate(item.expires_at)}
              </span>
            )}
          </div>
        </div>
        ...
        <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "8px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "6px", padding: "10px 20px", backgroundColor: "#8b1a2a", color: "#fff", borderRadius: "3px", fontSize: "12px", fontWeight: "700", fontFamily: "var(--font-sans)", letterSpacing: "0.04em", whiteSpace: "nowrap" as const }}>
            Lihat Detail
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
          </div>
          <span style={{ fontSize: "11px", color: "#9a9690", fontFamily: "var(--font-sans)" }}>
            {item.views.toLocaleString("id-ID")} tayangan
          </span>
        </div>
      </article>
    </Link>
...
function CardSkeleton() {
  return (
    <div style={{ backgroundColor: "#fff", border: "1px solid #e5e2dd", borderRadius: "4px", padding: "24px 28px", display: "flex", gap: "24px", alignItems: "center" }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "10px" }}>
        <div className="skeleton" style={{ height: "10px", width: "12%", borderRadius: "2px" }} />
        <div className="skeleton" style={{ height: "18px", width: "55%", borderRadius: "2px" }} />
        <div className="skeleton" style={{ height: "13px", borderRadius: "2px" }} />
        <div className="skeleton" style={{ height: "13px", width: "70%", borderRadius: "2px" }} />
        <div className="skeleton" style={{ height: "11px", width: "35%", borderRadius: "2px", marginTop: "4px" }} />
      </div>
      <div className="skeleton" style={{ width: "108px", height: "38px", borderRadius: "3px", flexShrink: 0 }} />
    </div>
  );
}
...
export default function KarirPage() {
  const [items, setItems] = useState<Karir[]>([]);
  const [loading, setLoading] = useState(true);
  const [total, setTotal] = useState(0);

  useEffect(() => {
    fetch(`${BASE}/karir?per_page=50`)
      .then((r) => r.json())
      .then((json) => {
        if (json.success) {
          setItems(json.data);
