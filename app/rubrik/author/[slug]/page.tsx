...
function ArticleCard({ article }: { article: Article }) {
  ...
  return (
    <Link href={`/rubrik/${article.slug}`} style={{ textDecoration: "none", display: "block", height: "100%" }}>
      ...
      >
        <div style={{ height: "180px", backgroundColor: "#f0eeec", overflow: "hidden", borderBottom: "1px solid #e5e2dd", flexShrink: 0 }}>
...
export default function AuthorPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/articles/author/${slug}?per_page=${PER_PAGE}&page=${currentPage}`)
      ...
      .then((json) => {
        if (json.success) {
          // Set author from meta
          if (json.meta?.author) setAuthor(json.meta.author);
    ...
    return (
      <div style={{ backgroundColor: "#f8f7f5", minHeight: "60vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 24px" }}>
        <div style={{ textAlign: "center" }}>
          ...
          <h2 style={{ fontSize: "24px", fontWeight: "700", color: "#0d0d0d", fontFamily: "var(--font-serif)", margin: "16px 0 8px" }}>Penulis Tidak Ditemukan</h2>
          <p style={{ fontSize: "14px", color: "#9a9690", fontFamily: "var(--font-sans)", marginBottom: "24px" }}>Penulis yang kamu cari tidak ditemukan atau belum memiliki artikel.</p>
  ...
  return (
    <>
      {/* ── HERO ──────────────────────────────────────────────────────────── */}
      <section style={{ backgroundColor: "#0d0d0d", padding: "48px 24px 40px", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0, backgroundImage: `linear-gradient(rgba(255,255,255,0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.02) 1px, transparent 1px)`, backgroundSize: "40px 40px", pointerEvents: "none" }} />
        <div style={{ position: "absolute", top: "-80px", right: "-80px", width: "300px", height: "300px", borderRadius: "50%", background: "radial-gradient(circle, rgba(139,26,42,0.12) 0%, transparent 70%)", pointerEvents: "none" }} />
        ...
        <div style={{ maxWidth: "1280px", margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "24px", flexWrap: "wrap" as const }}>
            ...
            <div>
              ...
              {loading ? (
                <div className="skeleton" style={{ height: "32px", width: "220px", borderRadius: "2px", marginTop: "6px" }} />
              ) : (
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "24px", paddingBottom: "12px", borderBottom: "3px solid #0d0d0d" }}>
            <div>
              ...
              <h2 style={{ fontSize: "18px", fontWeight: "700", color: "#0d0d0d", fontFamily: "var(--font-serif)", marginTop: "2px", marginBottom: 0 }}>
                Artikel oleh {loading ? "..." : authorName}
              </h2>
            </div>
            {meta && (
