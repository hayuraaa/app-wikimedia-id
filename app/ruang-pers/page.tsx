...
export default function RuangPersPage() {
  ...
  const fetchData = useCallback(async (page: number, query: string) => {
    ...
    try {
      let url = "";
      if (query) {
        url = `${BASE}/press-releases/search/query?q=${encodeURIComponent(query)}&per_page=${PER_PAGE}&page=${page}`;
      } else {
        url = `${BASE}/press-releases?per_page=${PER_PAGE}&page=${page}`;
      }
